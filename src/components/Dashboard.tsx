...
export default function Dashboard({ viewTimezone, setActiveTab, setEditingTicketId }: DashboardProps) {
  ...
  return (
    <div className="flex flex-col gap-8 pb-24 md:pb-8 animate-in fade-in duration-200">
      
      {/* Dashboard Top Banner */}
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-5 border-b border-slate-200 dark:border-slate-800 pb-5">
        <div>
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white uppercase flex items-center gap-2 mt-1">
            ✈ FLIGHT DEPARTURES
          </h1>
      ...
      {/* FILTER PANEL */}
      <div className="glass rounded-2xl p-5 md:p-6 flex flex-col gap-5 shadow-sm">
      ...
      <div className="flex items-center gap-4 border-b border-slate-200 dark:border-slate-800">
        {(['all', 'today', 'tomorrow', 'upcoming', 'missed'] as const).map(tab => (
          <button
            ...
            className={`px-1 py-3 text-xs font-semibold border-b-2 capitalize transition-all ${
              departuresTab === tab 
          ...
          <div className="md:hidden flex flex-col gap-4">
            {filteredTickets.map(ticket => {
              ...
              return (
                <div 
                  key={ticket.id} 
                  className="glass rounded-xl p-5 flex flex-col gap-4 shadow-sm border border-[var(--card-border)] relative overflow-hidden active:scale-[0.99] transition-all"
                >
                  {/* Top card row */}
                  <div className="flex items-start justify-between">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-bold text-sm text-slate-900 dark:text-white leading-tight uppercase tracking-tight">
                          {ticket.passenger_name.toUpperCase()}
                        </span>
                      </div>
                      <span className="text-[10px] text-slate-400 font-medium tracking-tight">
                        PNR: <span className="font-bold font-mono text-primary dark:text-accent">{ticket.pnr}</span> | Flight: {ticket.flight_number}
                      </span>
                    </div>
                    <span className={`px-2.5 py-0.5 rounded-full text-[9px] font-bold border ${getStatusBadge(ticket.status)}`}>
                      {ticket.status}
                    </span>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-b border-slate-100 dark:border-slate-800/80 py-3">
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <span className="text-[10px] text-slate-400 uppercase font-bold">Zoned Departure</span>
                  ...
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] text-slate-400">Created by: {ticket.created_by === 'usr-1' ? 'Lars (Admin)' : 'Staff'}</span>
                    
                    <div className="flex items-center gap-2">
                      <button 
                        onClick={() => setSelectedTicket(ticket)}
                        className="px-3 py-1.5 rounded-lg bg-slate-100 dark:bg-slate-800 text-[10px] font-bold text-slate-700 dark:text-slate-300 flex items-center gap-1.5"
                      >
                      ...
                      <button 
                        onClick={() => setShowReminderModal(ticket)}
                        className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary dark:bg-accent/15 dark:text-accent text-[10px] font-bold flex items-center gap-1.5"
                      >
                      ...
                      {ticket.status !== 'Missed' && ticket.status !== 'Archived' && (
                        <button 
                          onClick={() => handleMarkMissed(ticket.id)}
                          className="px-3 py-1.5 rounded-lg bg-amber-500/20 text-amber-400 text-[10px] font-bold flex items-center gap-1.5"
                        >
                          Missed
                        </button>
                      )}
      ...
      {selectedTicket && (
        <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="glass rounded-3xl w-full max-w-2xl max-h-[90vh] overflow-y-auto shadow-2xl p-6 border border-white/10 flex flex-col gap-6 animate-in zoom-in-95 duration-200">
